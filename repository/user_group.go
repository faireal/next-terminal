...
func (r UserGroupRepository) FindAll() (o []models.UserGroup) {
...
func (r UserGroupRepository) Find(pageIndex, pageSize int, name, order, field string) (o []models.UserGroupForPage, total int64, err error) {
	db := r.DB.Table("user_groups").Select("user_groups.id, user_groups.name, user_groups.created_at, count(resource_sharers.user_group_id) as asset_count").Joins("left join resource_sharers on user_groups.id = resource_sharers.user_group_id and resource_sharers.resource_type = 'asset'").Group("user_groups.id")
	...
	if o == nil {
		o = make([]models.UserGroupForPage, 0)
	}
	return
}

func (r UserGroupRepository) FindById(id string) (o models.UserGroup, err error) {
...
func (r UserGroupRepository) Create(o *models.UserGroup, members []string) (err error) {
...
func (r UserGroupRepository) Update(o *models.UserGroup, members []string, id string) error {
	return r.DB.Transaction(func(tx *gorm.DB) error {
		o.ID = id
		...
		err = tx.Where("user_group_id = ?", id).Delete(&models.UserGroupMember{}).Error
		if err != nil {
			return err
		}
		if members != nil {
...
func (r UserGroupRepository) DeleteById(id string) (err error) {
	err = r.DB.Where("id = ?", id).Delete(&models.UserGroup{}).Error
	if err != nil {
		return err
	}
	return r.DB.Where("user_group_id = ?", id).Delete(&models.UserGroupMember{}).Error
}
