...
func (r SessionRepository) Find(pageIndex, pageSize int, status, userId, clientIp, assetId, protocol string) (results []models.SessionForPage, total int64, err error) {
	...
	if results == nil {
		results = make([]models.SessionForPage, 0)
	}
	return
}

func (r SessionRepository) FindByStatus(status string) (o []models.Session, err error) {
	err = r.DB.Where("status = ?", status).Find(&o).Error
	return
}

func (r SessionRepository) FindByStatusIn(statuses []string) (o []models.Session, err error) {
...
func (r SessionRepository) Create(o *models.Session) (err error) {
	err = r.DB.Create(o).Error
	return
}

func (r SessionRepository) FindById(id string) (o models.Session, err error) {
	err = r.DB.Where("id = ?", id).First(&o).Error
	return
}

func (r SessionRepository) FindByIdAndDecrypt(id string) (o models.Session, err error) {
	err = r.DB.Where("id = ?", id).First(&o).Error
	if err == nil {
		err = r.Decrypt(&o)
	}
...
func (r SessionRepository) Decrypt(item *models.Session) error {
...
func (r SessionRepository) FindByConnectionId(connectionId string) (o models.Session, err error) {
	err = r.DB.Where("connection_id = ?", connectionId).First(&o).Error
	return
}

func (r SessionRepository) UpdateById(o *models.Session, id string) error {
	o.ID = id
...
func (r SessionRepository) DeleteByStatus(status string) error {
	return r.DB.Where("status = ?", status).Delete(&models.Session{}).Error
}
